...
The homepage includes a category multi-select and publisher selector to narrow the catalog. Select one or more categories, a publisher, or both; use **Clear filters** to restore the full list. Filtering happens in the browser over the statically generated game cards, so it does not require a backend service or change the URL.
...
The repository's coding standards are maintained in the [Copilot instructions](.github/copilot-instructions.md) and the focused [Astro](.github/instructions/astro.instructions.md), [data layer](.github/instructions/drizzle.instructions.md), [UI](.github/instructions/ui.instructions.md), and [styling](.github/instructions/style.instructions.md) instruction files. In particular, comments should explain intent rather than restate code; exported data-layer functions require TSDoc/JSDoc contracts; reusable Astro components must document their `Props` interfaces; and TypeScript semicolon and quote conventions are enforced by ESLint.
